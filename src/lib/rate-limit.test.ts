...
describe("RATE_LIMITS presets", () => {
  it("send and react budgets are independent", async () => {
    __resetRateLimitForTests();
    // Importing here so the presets stay close to their assertions.
    const { RATE_LIMITS } = await import("./rate-limit");
    expect(RATE_LIMITS.react.limit).toBeGreaterThan(RATE_LIMITS.send.limit);
    expect(RATE_LIMITS.send.windowMs).toBe(60_000);
    expect(RATE_LIMITS.react.windowMs).toBe(60_000);
  });
});
